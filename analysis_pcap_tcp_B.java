...
class analysis_pcap_tcp_B{
	...
	public static void outputPacketInfo(ArrayList<Integer>theFlowPortNums, long[][]table, int length){
		...
		for(int q = 0;q<theFlowPortNums.size();q++){
			ArrayList<Long> darn = new ArrayList<Long>();
			ArrayList<Integer> ind = new ArrayList<Integer>();
			long curWind = 0;
			System.out.println("Flow " + q + ":");
			while(i<12 && i<table.length){
				
				if(theFlowPortNums.get(q)== table[j][3] || theFlowPortNums.get(q)==table[j][4]){
					
					if(table[j][2]!=curWind){
						curWind=table[j][2];
						darn.add(table[j][2]);
						ind.add(j);
						i++;
					}
					if(!darn.contains(table[j][2])){
			...
			System.out.println(darn.toString());
